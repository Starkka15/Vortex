          ...
          fs
            ...
            .catch({ code: "ENOENT" }, (err) => {
              api.showErrorNotification("Failed to write ini file", err, {
                ...
                attachments: [
                  {
                    id: path.basename(iniFileName) + ".base",
                    type: "data",
                    data: delta,
              ...
              .then(() =>
                fs
                  ...
                  .catch((err) =>
                    err.code === "ENOENT"
                      ? fs
                          .copyAsync(iniFileName, iniFileName + ".base", {
                            noSelfCopy: true,
                          })
                          .then(() =>
                            fs.copyAsync(iniFileName, iniFileName + ".baked", {
                              noSelfCopy: true,
                            }),
                          )
                          .then(() =>
          ...
          .then((ini) =>
            PromiseBB.each(enabledTweaks[baseName] || [], (tweak) =>
              ...
              .then(() => {
                ...
                return fs.copyAsync(iniFileName + ".baked", iniFileName, {
                  noSelfCopy: true,
                });
              }),
          );
...
function testProtectedFolderAccess(): PromiseBB<ITestResult> {
  ...
  try {
    ...
    log(
      "error",
      "Unable to get path to my documents folder - user environment is misconfigured!",
      err,
    );
    ...
    .catch((err) => {
      ...
      return {
        description: {
          ...
          long:
            "Vortex is being blocked from running file operations by your Anti-Virus software " +
