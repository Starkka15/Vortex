...
public:
  LZ4Decoder(std::shared_ptr<IDecoder> &wrapee, uint32_t compressedSize, uint32_t uncompressedSize)
  {
...
public:
  ZlibDecoder(std::shared_ptr<IDecoder> &wrapee, uint32_t compressedSize, uint32_t uncompressedSize)
  {
    std::string tempCompressed;
    tempCompressed.resize(compressedSize);
...
void GamebryoSaveGame::readFO3(GamebryoSaveGame::FileWrapper &file)
{
  ...
  file.skip<uint32_t>(); //File version? always 0x30
...
void GamebryoSaveGame::FileWrapper::readImage(uint32_t width, uint32_t height, bool alpha)
{
...
void GamebryoSaveGame::FileWrapper::setCompression(unsigned short format, uint32_t compressedSize, uint32_t uncompressedSize)
{
  if (format == 1) {
